...
namespace EndplayCheck
{
    public class TfSQL
    {
        ...
        public TfSQL()
        {
            ...
            string server = sto.server;
            string database = sto.database;
            string user = sto.user;
            string pass = sto.pass;
            strConnection = @"Server= " + server + "; Port=5432;"
        ...
        public bool sqlMulSernoInsert(string table, string serno, string lot, string model, string site, string factory,
            string line, string process, DateTime inspectdate, string judge, string status, string remark, DataTable dt)
        {
            int res1;
            bool res2 = false;
            connection = new NpgsqlConnection(strConnection);
            ...
            try
            {
                // ＤＢテーブルに既に存在するデータを削除し、登録。ただし例外発生時、削除も登録もロールバック。
                // 削除①：
                string sql1 = "delete from " + table + " where " +
                    "model ='" + model + "' and " +
                    "lot ='" + lot + "' and " +
                    "inspectdate ='" + inspectdate + "' and " +
                    "line ='" + line + "'";
                ...
                string sql2 = @"INSERT INTO " + table +
                           "(serno, lot, model, site, factory, line, process, inspectdate, tjudge, tstatus, remark) " +
                           "VALUES " +
                           "(:serno, :lot, :model, :site, :factory, :line, :process, :inspectdate, :tjudge, :tstatus, :remark)";
                NpgsqlCommand command2 = new NpgsqlCommand(sql2, connection);

                command2.Parameters.Add(new NpgsqlParameter("serno", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("lot", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("model", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("site", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("factory", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("line", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("process", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("inspectdate", NpgsqlTypes.NpgsqlDbType.TimestampTZ));
                command2.Parameters.Add(new NpgsqlParameter("tjudge", NpgsqlTypes.NpgsqlDbType.Varchar));
                command2.Parameters.Add(new NpgsqlParameter("tstatus", NpgsqlTypes.NpgsqlDbType.Double));
                command2.Parameters.Add(new NpgsqlParameter("remark", NpgsqlTypes.NpgsqlDbType.Double));
                int k = 1;
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    command2.Parameters[0].Value = dt.Rows[i]["serno"].ToString();
                    command2.Parameters[1].Value = dt.Rows[i]["lot"].ToString();
                    command2.Parameters[2].Value = dt.Rows[i]["model"].ToString();
                    command2.Parameters[3].Value = dt.Rows[i]["site"].ToString();
                    command2.Parameters[4].Value = dt.Rows[i]["factory"].ToString();
                    command2.Parameters[5].Value = dt.Rows[i]["line"].ToString();
                    command2.Parameters[6].Value = dt.Rows[i]["process"].ToString();
                    command2.Parameters[7].Value = dt.Rows[i]["inspectdate"].ToString();
                    command2.Parameters[8].Value = dt.Rows[i]["judge"].ToString();
                    command2.Parameters[9].Value = dt.Rows[i]["status"].ToString();
                    command2.Parameters[10].Value = dt.Rows[i]["remark"].ToString();
