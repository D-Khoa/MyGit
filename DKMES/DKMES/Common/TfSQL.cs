...
namespace DKMES.Common
{
    public class TfSQL
    {
        ...
        //public TfSQL()
        //{
        //    //strConnection = Properties.Settings.Default.MESDB_CONNECTION;
        //}
        ...
        public void getComboBoxData(string sql, ref ComboBox cmb, bool autocomplete)
        {
            ...
            try
            {
                ...
                {
                    cmb.Items.Add(row[0].ToString());
                    if (autocomplete) DataCollection.Add(row[0].ToString());
                }
                connection.Close();
                if (autocomplete) cmb.AutoCompleteCustomSource = DataCollection;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Database Responce", MessageBoxButtons.OK, MessageBoxIcon.Warning);
